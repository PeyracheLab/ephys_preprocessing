basePath function SleepState = SleepScoreMaster(basePath,varargin)
...
% fix from Guillaume to avoid loading the same function from TsToolbox
addpath(genpath('/home/adrien/Toolbox/buzcode'));
...
%% MAKE THE STATE SCORE OUTPUT FIGUREaddpath(genpath('/home/adrien/Toolbox/buzcode'));
...
% Saving for Neuroscope
[~,recordingname,~] = fileparts(pwd);
sws_ep = SleepState.ints.NREMstate;
fileId = fopen([recordingname '.sws.evt'], 'w');
[n,m] = size(sws_ep);
for i=1:n
   fprintf(fileId, [int2str(sws_ep(i,1)*1000) ' SWS start 1\n']);
   fprintf(fileId, [int2str(sws_ep(i,2)*1000) ' SWS end 1\n']);
end
fclose(fileId);

rem_ep = SleepState.ints.REMstate;
fileId = fopen([recordingname '.rem.evt'], 'w');
[n,m] = size(rem_ep);
for i=1:n
   fprintf(fileId, [int2str(rem_ep(i,1)*1000) ' REM start 1\n']);
   fprintf(fileId, [int2str(rem_ep(i,2)*1000) ' REM end 1\n']);
end
fclose(fileId);

% fix from Guillaume to avoid loading the same function from TsToolbox
rmpath(genpath('/home/adrien/Toolbox/buzcode'));
addpath(genpath('/home/adrien/Toolbox/buzcode/detectors/detectStates/SleepScoreMaster')); %dirty
